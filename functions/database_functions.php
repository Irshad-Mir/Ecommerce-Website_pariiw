<?php
	function db_connect(){
		$conn = mysqli_connect("127.0.0.1", "root", "", "db_pariiw");
		if(!$conn){
	...
	function select8LatestBook($conn){
		$row = array();
		$query = "SELECT book_isbn, book_image,book_title FROM books ORDER BY book_isbn DESC";
...
function searchKeyword($conn){
    if(isset($_GET['keywords']))
         $keywords=($_GET['keywords']);
		$row = array();
		$query = "SELECT book_title,book_descr,book_image,book_price,book_isbn FROM books WHERE book_title LIKE '%{$keywords}%' OR book_descr LIKE '%{$keywords}%'  ORDER BY book_isbn DESC";
		$result = mysqli_query($conn, $query);
		if(!$result){
		    echo "Can't retrieve data " . mysqli_error($conn);
		    exit;
